...
export class RulesComponent {
  ...
  savingRuleCode: string | null = null;
  ...
  save(rule: RuleConfig) {
    if (!rule.ruleCode.trim() || !this.valid(rule) || this.savingRuleCode) return;
    this.savingRuleCode = rule.ruleCode;
    const payload = { thresholdAmount: rule.thresholdAmount, windowMinutes: rule.windowMinutes, maxCount: rule.maxCount, isEnabled: rule.isEnabled, percentageDeviation: rule.percentageDeviation, customParametersJson: rule.customParametersJson };
    this.api.updateRule(rule.ruleCode, payload).subscribe({
      next: () => {
        this.savingRuleCode = null;
        this.toast.show(`${rule.ruleCode} updated.`);
        this.changeDetector.detectChanges();
      },
      error: error => {
        this.savingRuleCode = null;
        this.toast.show(error.error?.message || 'Could not update rule.', 'danger');
        this.changeDetector.detectChanges();
      }
    });
  }
